  ...
  return [
    {
      label: "รายวัน",
      variable: "daily",
      amount: Math.ceil(price / daysIn6Months),
    },
    {
      label: "รายสัปดาห์",
      variable: "weekly",
      amount: Math.ceil(price / (daysIn6Months / 7)),
    },
    {
      label: "ราย 15 วัน",
      variable: "15",
      amount: Math.ceil(price / (daysIn6Months / 15)),
    },
...
export default function ChangeGoalPage({ className, isEditing, setIsEditing }) {
  ...
  return (
    <FramerDiv
      ...
      className={clsx(
        "bg-bg-dark/80 fixed inset-0 z-50 flex flex-col backdrop-blur-sm",
        className,
      )}
    >
      {/* Page Header */}
      ...
      <div className="relative flex-grow overflow-y-auto rounded-t-[30px] bg-white p-6 pb-28">
          ...
          <div id="edit-phone-list" className="grid grid-cols-1 gap-4">
            {phones.map((phone) => (
              <GoalPhoneComponent
                ...
                handleSelected={(details) =>
                  setNewGoal({ ...details, plan: null })
                }
        ...
        <div
          className={clsx("border-divider border-t pt-5", {
            hidden: !newGoal.brand,
          })}
        >
          <h2 className="mb-4 text-lg font-bold text-neutral-800">
            2. เลือกแผนการออมใหม่ (ระยะเวลา 6 เดือน)
          </h2>
          <div id="edit-savings-plan-list" className="grid grid-cols-2 gap-3">
            {savingsPlans.map((plan) => (
              <div
                ...
                onClick={() =>
                  setNewGoal((prev) => ({ ...prev, plan: plan.variable }))
                }
              >
      ...
      <div className="absolute right-0 bottom-0 left-0 bg-white p-6 pt-5 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
        <CtaButton disabled={newGoal.plan === null}>
